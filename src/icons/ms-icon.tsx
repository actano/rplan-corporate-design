...
const getContent = (type: MilestoneIconType, backgroundColor: string) => {
  let result
  switch (type) {
    case MilestoneIconType.FLEXIBLE:
      result = (
        <>
          ...
          <path
            ...
            strokeWidth="2"
          />
          <rect x="9.99999" y="5" width="6" height="15.5" rx="3" fill={backgroundColor} stroke="none" />
        </>
      )
      break
...
const MsIcon = React.forwardRef<any, MsIconProps>(({
  ...
  return (
    ...
    >
      {
        getContent(type, backgroundColor)
      }
    </SvgIcon>
  )
})
